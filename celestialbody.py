##########################################################################
# celestialbody.py offers a way to compute fast approximate ephemerides
...
unnum_asteroids_filename = "ELEMENTS.UNNUM"
...
def read_unnumbered_asteroid_params(row):
    """
        read a given row from "ELEMENTS.UNNUM"
    """
    unnum_asteroid = dict(
        category="unnumbered_asteroid",
        modified_epoch=int(row[14:19].strip(" ")),
        # Values
        name=row[:13].strip(" "),
        semimajor_axis=float(row[20:31].strip(" ")),
        eccentricity=float(row[32:42].strip(" ")),
        inclination=float(row[43:52].strip(" ")),
        perihelion_argument=float(row[53:62].strip(" ")),
        ascending_node_longitude=float(row[63:72].strip(" ")),
        mean_anomaly=float(row[73:84].strip(" "))
    )
    return unnum_asteroid

def update_unnumbered_asteroid_params(name):
    """
        updates params from a specified numbered asteroid from ELEMENTS.UNNUM
        data are available at https://ssd.jpl.nasa.gov/?sb_elem
        return: dict with unnumbered asteroid params
    """
    filename = unnum_asteroids_filename
    folder = os.path.dirname(__file__) + "/"
    with open(folder + filename) as file:
        content = file.readlines()
    for row in content:
        if name in row:
            break
    unnum_asteroid = read_unnumbered_asteroid_params(row)
    unnum_asteroid["epoch"] = julian_ephemeris_date_to_datetime(unnum_asteroid["modified_epoch"] + 2400000.5)
    unnum_asteroid["perihelion_longitude"] = unnum_asteroid["ascending_node_longitude"] + unnum_asteroid[
        "perihelion_argument"]
    unnum_asteroid["mean_longitude"] = unnum_asteroid["mean_anomaly"] + unnum_asteroid["perihelion_longitude"]
    return unnum_asteroid

def unnumbered_asteroids_list():
    filename = unnum_asteroids_filename
    folder = os.path.dirname(__file__) + "/"
    with open(folder + filename) as file:
        content = file.readlines()
...
UNNUMBERED_ASTEROIDS = unnumbered_asteroids_list()
COMETS    = comets_list()
BODIES    = PLANETS + ASTEROIDS + UNNUMBERED_ASTEROIDS + COMETS
...
class CelestialBody():
    ...
    def __init__(self, name, category=None, verbose=False):
        ...
        if category is None:
            ...
            elif data_base_name in UNNUMBERED_ASTEROIDS:
...
def collect_param(name, category):
    """
    Collect parameter name (a, e or i) in category (planet bodies
    """
    if category == "planet":
        filename = planets_filename
    elif category == "asteroid":
    ...
    if category == "planet":
        for planet in PLANETS:
            body = update_planet_params(planet)
            semimajor_axis = body["semimajor_axis"]
        ...
        for row in content[2:]:
            ...
            inclination = float(row[75:84].strip(" "))
            if eccentricity == 1:
                semimajor_axis = np.infty
            else:
                semimajor_axis = perihelion_distance / (1 - eccentricity)
            semimajor_axes.append(semimajor_axis)
            eccentricities.append(eccentricity)
            inclinations.append(inclination)
    if name == "a":
        params = semimajor_axes
    elif name == "e":
        params = eccentricities
    elif name == "i":
        params = inclinations
    else:
